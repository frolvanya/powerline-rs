...
fn statuses_if_none(git: &Repository, statuses: &mut Option<Vec<Status>>) -> bool {
    ...
    if statuses.is_none() {
        *statuses = git
            ...
            .ok()
            .map(|statuses| statuses.iter().map(|entry| entry.status()).collect());
        statuses.is_some()
    } else {
        true
    }
    ...
    if branch_name.is_none() {
        ...
        if let Ok(head) = git.head() {
            ...
            p.segments.push(Segment::new(
                p.theme.git_dirty_bg,
                p.theme.git_dirty_fg,
                "Big Bang",
            ));
            return;
        }
        ...
        if let Some(upstream) = upstream {
            if let Ok((ahead, behind)) = git.graph_ahead_behind(local, upstream) {
                if ahead > 0 {
                    ...
                    p.segments.push(Segment::new(
                        p.theme.git_ahead_bg,
                        p.theme.git_ahead_fg,
                        ahead,
                    ));
                }

                if behind > 0 {
                    let mut behind = if behind == 1 {
                        String::new()
                    } else {
                        behind.to_string()
                    };
                    ...
                    p.segments.push(Segment::new(
                        p.theme.git_behind_bg,
                        p.theme.git_behind_fg,
                        behind,
                    ));
                }
            }
...
pub fn segment_gitstage(p: &mut Powerline) {
    ...
    if staged > 0 {
        ...
        p.segments.push(Segment::new(
            p.theme.git_staged_bg,
            p.theme.git_staged_fg,
            string,
        ));
    }
    if notstaged > 0 {
        let mut string = if notstaged == 1 {
            String::with_capacity(1)
        } else {
            notstaged.to_string()
        };
        ...
        p.segments.push(Segment::new(
            p.theme.git_notstaged_bg,
            p.theme.git_notstaged_fg,
            string,
        ));
    }
    if untracked > 0 {
        let mut string = if untracked == 1 {
            String::with_capacity(1)
        } else {
            untracked.to_string()
        };
        ...
        p.segments.push(Segment::new(
            p.theme.git_untracked_bg,
            p.theme.git_untracked_fg,
            string.clone(),
        ));
    }
    if conflicted > 0 {
        let mut string = if conflicted == 1 {
            String::with_capacity(1)
        } else {
        ...
        p.segments.push(Segment::new(
            p.theme.git_conflicted_bg,
            p.theme.git_conflicted_fg,
            string,
        ));
    }
}
